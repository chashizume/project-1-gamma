...
df_startofcovid_gtrends <- 
  gtrends(
    c("toilet paper", "animal crossing", "tiger king", "dalgona coffee", "tiktok"), 
...
df_nyt_national_startofcovid <-
  df_nyt_national%>%
  filter(date <= "2020-06-01" & date >= "2020-03-01")

scale_factor <- max(df_nyt_national_startofcovid$dcases_dt) / max(df_startofcovid_gtrends$hits)

ggplot() +
geom_line(
  data = df_startofcovid_gtrends,
  mapping = aes(x = as.POSIXct(date), y = hits * scale_factor, color = keyword)
  ) +
  labs(
    x = "Date", 
    y = "Hits", 
...
ggplot() +
  geom_line(data = df_nyt_national_twoweeks, aes(x = as.POSIXct(date), y = dcases_dt, color = "Covid Cases"), linetype = "dashed") +
  geom_line(data = animal_crossing, aes(x = as.POSIXct(date), y = hits * scale_factor, color = keyword)) +
  scale_color_manual(
  labels = c( "Animal Crossing Searches","New Covid Cases in the US"),
...
```
Looking closer at a 2-week section of Animal crossing data vs Covid cases, we see an interesting trend. We are not sure the reasoning for it, but found it interesting and wanted to point it out. In the graph seen above, we see that when the number of cases spikes, searches for Animal Crossing are at a low point. One theory we have is that the extreme points occurred on weekends when people have more time to play games and covid cases were not counted. We would have to explore deeper to know for sure, but nonetheless, we found the data interesting.

### Financial Decisions during the Pandemic

During the past two years or so, the COVID-19 pandemic has been quite devastating for people all across the country in terms of health, job loss, and other risks onto their livelihood. However, we observed growth in other socio-economic metrics like the saving rate soaring at 33.7% in April
2020 [8]. Having more disposable income paired with the stimulus checks garnered interest in the stock markets lead all three key indexes (S&P500, NASDAQ, Dow Jones) up in the Q4 2020 and Q1 2021 average an increase of 17-27% [9]. In that regard, we wanted to examine if certain key search terms capture this new found enthusiasm for the average American for the stock market.

P.S: Reminder that we are working with aggregated data of searches and not prices for indexes or securities publicly traded.
...
In pulling the trends data, we explored a different terms and opted to highlights two phrases that are "stock market" as a direct reference to financial trading markets and "robinhood" as a popular platform facilitating trading for retail investors.
...
```{r}
scale_factor <- max(df_national_data$new_cases) / max(df_financial_gtrends$hits)

ggplot() +
  geom_line(data = df_national_data, aes(x = date, y = new_cases)) +
  geom_line(data = df_financial_gtrends, aes(x = date, y = hits * scale_factor, color = keyword)) +
...
-   As mentioned earlier, we observed changes in spending and savings habits being stuck at home and for some, having the ability to work remotely further cutting on transportation expenses for instances. From the disposable income, the hypothesis will be that this new found capital fueled this new wave of investors. So, the trends from the Google search would capture this interest as people try to inform themselves and get into or out of the market.

-   Indeed, we shall focus on two distinct periods: Q1 2020 marking the beginning of the pandemic and Q4 2020 with the new found optimism from the promising vaccine results.

-   In that first period, we note that the "stock market" term is the only having a spike. On one hand, some investors would have probably considered exiting the market out of panic while others would have the foresight of certain stocks preponderant of various services key during the pandemic with their new found capital. [10]

-   In the second period, there was the announcement from Pfizer on their effectiveness of their vaccine at 90% which would have fostered new optimism among investors. Other factors would also be important to highlights that are grass-roots online movement like r/[WallStreetBets] community on Reddit. [9]

-   Overall, we cannot have state with certainty that the pandemic was the only factor, or even less to be denoted as the cause of those newly observed behaviors. However, from a financial analysis and forecasting perspective, this approach may offer an interesting route for further exploration.
...
NOTE: COVID data has been smoothed with a 7-day moving average to account for weekly inaccuracies due to test numbers decreasing on weekends.
...
The term 'vaccine 5g' was trending as early as March 15, as US COVID numbers were just starting to increase, and months before any viable vaccine was in human trials. This increase was likely related to news coverage on the topic which began as early as [May 18](https://maldita.es/malditaciencia/20200518/post-facebook-coronavirus-vacunas-chemtrails-5g/) but increased in early March [5]. The BBC did an analysis of [social media posts related to the topic](https://www.bbc.com/news/53191523) and found that they had substantial reach across the globe [6]. In this case it is actually interesting to note that searches do not seem to be well correlated with vaccination rates but rather with critical events in the vaccination timeline. We see a spike at the very beginning of the pandemic, a spike as large-scale vaccinations begin, and a few more spikes as vaccinations increase then taper off. Not all of the trends in searches can be explained by these events but they do seem to overlap with many of them.
...
We calculated the time-synchronized correlation between a number of search terms and common pandemic metric such as cases, deaths, and vaccinations. While many of these factors have little correlation, some stand out as being much more tightly coupled.
